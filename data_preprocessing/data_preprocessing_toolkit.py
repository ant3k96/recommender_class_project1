...
class DataPreprocessingToolkit(object):
    ...
    def add_night_price(self, df):
        df.loc[:, "night_price"] =  (df['accomodation_price']* df['n_rooms'])/df['length_of_stay']
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        ...
        df.loc[:, 'room_segment'] = df['termnight_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
